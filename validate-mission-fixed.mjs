...
async function main() {
  ...
  const { data: creators } = await supabase
    .from('creators')
    .select('id, email, user_id, discount_code')
    .order('email')
  ...
  // Test 3: Dashboard for each creator - using creator.id NOT user_id
  console.log('📊 TEST 3: Dashboard par créateur')
  for (const creator of creators) {
    const { data: dashboard, error } = await supabase
      .rpc('get_creator_dashboard', { p_creator_id: creator.id })  // Use creator.id!
    ...
    if (error) {
      ...
      const pending = dashboard?.pending_commissions?.count || 0
      const payable = dashboard?.payable_commissions?.count || 0
      console.log(`   ✅ ${creator.email}: ${balance.toFixed(2)}€, ${activity} activités, ${pending} pending, ${payable} payable`)
    }
  }
  console.log('')
  ...
  console.log('')
  console.log('🔐 Credentials pour tous:')
